...
from training_models import FFSpeechModel
from training_utils import compute_stats, TFSample, SpeechCommandsHandler

resume = False
rate = 0.0005
n_epochs = 8

sc_path = '/home/pblouw/git/abr_speech/data/speech_commands/'
ff_path = './checkpoints/speech_commands_model'

# load the audio files collected from turkers

data = SpeechCommandsHandler(sc_path)
data.load_audio()

text = set([x.text for x in data.test_data])
print(text)

with open('sc_data.pkl', 'wb') as pfile:
